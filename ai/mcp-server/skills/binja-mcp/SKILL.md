...
If you have bash access, consider the `binja-cli` skill's `bn` command instead — same server, same tools underneath, but shell-composable (`| jq`, `| grep`) instead of one MCP call per lookup. This skill still matters for clients with no shell, or when you specifically want MCP's typed tool schema.

## Tool tiers

Per ADR-0038, the old per-kind listing tools (`get_functions`, `get_symbols`, `get_types`, `get_sections`, `get_imports`, `get_exports`, `get_strings`) are gone, replaced by one consolidated `list(kind, name_regex=None, fields=None, limit=100, offset=0)` covering all seven — `kind` is one of `"functions"`, `"symbols"`, `"types"`, `"sections"`, `"imports"`, `"exports"`, `"strings"`. `name_regex` filters (substring or regex) before pagination; `fields` narrows/reorders columns. Every tool in this server (not just read/list — write, undo, destructive-write, debugging, scripting too) returns plain text by design (a header line + tab-separated records for listings, `key: value` lines for single records) — never JSON — to keep responses out of proportion to their information content; that's not configurable per-call. The one exception is `capture_screenshot`, which returns actual MCP image content since a screenshot has no meaningful text form.

| Tier | Default | Tools |
|---|---|---|
| Read | always on | `get_function`, `list`, `get_xrefs_to`, `get_xrefs_from` (both now take `limit`/`offset`), `get_type`, `get_data`, `search` |
...
**Get oriented in an unfamiliar binary.** `binary://metadata` for arch/entry/size, then `list(kind="functions")`, `list(kind="symbols")`, `list(kind="strings")`, `list(kind="imports")`/`list(kind="exports")` for an overview. Equivalent to sending yourself the `reverse-engineering` prompt.

**Understand one function.** `get_function(addr, il_level="hlil")` for readable pseudocode (fall back to `mlil`/`llil` for tight codegen questions), then `get_xrefs_to`/`get_xrefs_from` for callers/callees and `list(kind="strings")`/`get_data` for referenced constants. Equivalent to the `analyze-function` prompt.

**Hunt for crypto.** `search` for algorithm names and telltale constants (S-boxes, IV values) across functions/symbols/strings at once, or `list(kind="imports", name_regex="crypto|ssl|aes")` to filter one kind specifically, `get_function` on candidates. Equivalent to the `find-crypto` prompt.
...
**Resolve vague targets before writing.** "Rename this", "patch that call", "comment the loop" without an explicit address — find the target yourself first (`search`, `list(kind="functions")`, `get_xrefs_to/from`, or a screenshot if it's about on-screen position) and state what you resolved it to before calling a write tool. Don't silently write to a guessed address.
